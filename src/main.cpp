...
int main()
{
	...
	char path2[50];
	char path3[50];
    Mat IRMAT,DEPTHMAT,IRMAT1,DEPTHMAT1;
	...
	while(!protonect_shutdown)
	{
		...
		cv::Mat(ir->height, ir->width, CV_32FC1, ir->data).copyTo(irmat); //CV_32FC1
		cv::Mat(depth->height, depth->width, CV_32FC1, depth->data).copyTo(depthmat);//CV_32FC1

        Mat irMat(irmat/4500.0f);
        Mat depthMat(depthmat/4500.0f);
        cv::normalize(irMat,IRMAT,1,0,NORM_MINMAX);
        cv::normalize(depthMat,DEPTHMAT,1,0,NORM_MINMAX);
        IRMAT.convertTo(IRMAT1,CV_8U,255);
        DEPTHMAT.convertTo(DEPTHMAT1,CV_8U,255);

        cv::imshow("rgb", rgbmat);
		cv::imshow("ir", IRMAT1);
		cv::imshow("depth", DEPTHMAT1);
		...
		{
			m++;
			sprintf(path1,"../rgb/%d.png",m);
			sprintf(path2,"../ir/%d.png",m);
			sprintf(path3,"../depth/%d.png",m);

			string Path1(path1);
			string Path2(path2);
			string Path3(path3);

			imwrite(Path1,rgbmat);
			imwrite(Path2,IRMAT1);
			imwrite(Path3,DEPTHMAT1);
		}
